...
function loadConfigs() {
  ...
  if (saved) {
    try {
      ...
      if (parsed && parsed.apis && parsed.apis.length >= DEFAULT_CONFIGS.apis.length - 1) {
...
function getCategoryLabel(cat) {
  const m = { mapping:'Mapas', realtime:'Realtime', payments:'Pagos', communications:'Coms', authentication:'Auth', storage:'Storage', notifications:'Push', streaming:'Stream', geospatial:'Geo', infrastructure:'Infra', database:'DB', 'artificial-intelligence':'IA/Gemini', verification:'Verif' };
  return m[cat] || cat;
}
